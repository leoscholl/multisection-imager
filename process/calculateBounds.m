function [globalPosY, globalPosX, inBounds, offset] = calculateBounds(imdata, size, roi)
...
inBoundsY = mean(globalPosY,1) + size(2) >= roi(2) & mean(globalPosY,1) < roi(2) + roi(4);
inBoundsX = mean(globalPosX,1) + size(1) >= roi(1) & mean(globalPosX,1) < roi(1) + roi(3);
